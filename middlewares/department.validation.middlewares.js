const { z } = require("zod");

// Base schema for required fields
const departmentSchema = z.object({
    company_id: z.coerce.number().int().positive("Company ID must be a positive integer."),
    name: z.string().min(2, "Department name must be at least 2 characters long."),
});

// ID Validation for URL parameters
const idSchema = z.object({
    id: z.string().refine((val) => {
        const num = Number(val);
        return !isNaN(num) && Number.isInteger(num) && num > 0;
    }, {
        message: "ID must be a positive integer in string format.",
    }),
});


// Middleware for Create Department
exports.validateCreateDepartment = (req, res, next) => {
    try {
        departmentSchema.parse(req.body);
        next();
    } catch (err) {
        return res.status(400).json({
            success: false,
            message: err.errors ? err.errors[0].message : err.message,
        });
    }
};

// Middleware for Update Department 
exports.validateUpdateDepartment = (req, res, next) => {
    try {
        const updateSchema = z.object({
            name: z.string().min(2, "Department name must be at least 2 characters long."),
        });
        
        updateSchema.parse(req.body);
        
        if (Object.keys(req.body).length === 0) {
            return res.status(400).json({
                success: false,
                message: "No fields provided for update. Only 'name' can be updated.",
            });
        }
        next();
    } catch (err) {
        return res.status(400).json({
            success: false,
            message: err.errors ? err.errors[0].message : err.message,
        });
    }
};

// Middleware for ID validation
exports.validateDepartmentId = (req, res, next) => {
    try {
        idSchema.parse(req.params);
        next();
    } catch (err) {
        return res.status(400).json({
            success: false,
            message: "Invalid Department ID format in parameters.",
        });
    }
};